.class public Landroid/graphics/drawable/AnimationDrawable;
.super Landroid/graphics/drawable/DrawableContainer;
.source "AnimationDrawable.java"

# interfaces
.implements Ljava/lang/Runnable;
.implements Landroid/graphics/drawable/Animatable;


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Landroid/graphics/drawable/AnimationDrawable$1;,
        Landroid/graphics/drawable/AnimationDrawable$AnimationState;
    }
.end annotation


# instance fields
.field final DEBUG_SCALED_ANIM:Z

.field final SCALED_MAX_LEVEL:I

.field private final mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

.field mBaseLevel:I

.field mBaseWidth:I

.field private mCurFrame:I

.field mCurWidth:I

.field mCurrDrawable:Landroid/graphics/drawable/Drawable;

.field mIncX:I

.field mIsScaledAnimate:Z

.field private mMutated:Z


# direct methods
.method public constructor <init>()V
    .locals 1

    .prologue
    const/4 v0, 0x0

    .line 100
    invoke-direct {p0, v0, v0}, Landroid/graphics/drawable/AnimationDrawable;-><init>(Landroid/graphics/drawable/AnimationDrawable$AnimationState;Landroid/content/res/Resources;)V

    .line 101
    return-void
.end method

.method private constructor <init>(Landroid/graphics/drawable/AnimationDrawable$AnimationState;Landroid/content/res/Resources;)V
    .locals 4
    .parameter "state"
    .parameter "res"

    .prologue
    const/4 v3, 0x1

    const/4 v1, -0x1

    const/4 v2, 0x0

    .line 544
    invoke-direct {p0}, Landroid/graphics/drawable/DrawableContainer;-><init>()V

    .line 86
    iput v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurFrame:I

    .line 90
    iput-boolean v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mIsScaledAnimate:Z

    .line 92
    iput v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    .line 93
    iput v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mIncX:I

    .line 94
    iput v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    .line 96
    const/16 v1, 0x64

    iput v1, p0, Landroid/graphics/drawable/AnimationDrawable;->SCALED_MAX_LEVEL:I

    .line 97
    iput-boolean v2, p0, Landroid/graphics/drawable/AnimationDrawable;->DEBUG_SCALED_ANIM:Z

    .line 545
    new-instance v0, Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    invoke-direct {v0, p1, p0, p2}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;-><init>(Landroid/graphics/drawable/AnimationDrawable$AnimationState;Landroid/graphics/drawable/AnimationDrawable;Landroid/content/res/Resources;)V

    .line 546
    .local v0, as:Landroid/graphics/drawable/AnimationDrawable$AnimationState;
    iput-object v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    .line 547
    invoke-virtual {p0, v0}, Landroid/graphics/drawable/AnimationDrawable;->setConstantState(Landroid/graphics/drawable/DrawableContainer$DrawableContainerState;)V

    .line 548
    if-eqz p1, :cond_0

    .line 549
    invoke-direct {p0, v2, v3, v2}, Landroid/graphics/drawable/AnimationDrawable;->setFrame(IZZ)V

    .line 551
    :cond_0
    return-void
.end method

.method synthetic constructor <init>(Landroid/graphics/drawable/AnimationDrawable$AnimationState;Landroid/content/res/Resources;Landroid/graphics/drawable/AnimationDrawable$1;)V
    .locals 0
    .parameter "x0"
    .parameter "x1"
    .parameter "x2"

    .prologue
    .line 84
    invoke-direct {p0, p1, p2}, Landroid/graphics/drawable/AnimationDrawable;-><init>(Landroid/graphics/drawable/AnimationDrawable$AnimationState;Landroid/content/res/Resources;)V

    return-void
.end method

.method private nextFrame(Z)V
    .locals 5
    .parameter "unschedule"

    .prologue
    .line 286
    iget v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurFrame:I

    add-int/lit8 v1, v3, 0x1

    .line 287
    .local v1, next:I
    iget-object v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    invoke-virtual {v3}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->getChildCount()I

    move-result v0

    .line 288
    .local v0, N:I
    if-lt v1, v0, :cond_0

    .line 289
    const/4 v1, 0x0

    .line 292
    :cond_0
    iget-boolean v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mIsScaledAnimate:Z

    if-eqz v3, :cond_2

    .line 294
    const/4 v1, 0x0

    .line 295
    invoke-virtual {p0}, Landroid/graphics/drawable/AnimationDrawable;->getBounds()Landroid/graphics/Rect;

    move-result-object v3

    iget v3, v3, Landroid/graphics/Rect;->right:I

    invoke-virtual {p0}, Landroid/graphics/drawable/AnimationDrawable;->getBounds()Landroid/graphics/Rect;

    move-result-object v4

    iget v4, v4, Landroid/graphics/Rect;->left:I

    sub-int v2, v3, v4

    .line 296
    .local v2, w:I
    iget v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    if-ltz v3, :cond_1

    iget v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    if-lt v3, v2, :cond_4

    .line 298
    :cond_1
    iget v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    iput v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    .line 310
    .end local v2           #w:I
    :cond_2
    :goto_0
    iget-object v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    #getter for: Landroid/graphics/drawable/AnimationDrawable$AnimationState;->mOneShot:Z
    invoke-static {v3}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->access$100(Landroid/graphics/drawable/AnimationDrawable$AnimationState;)Z

    move-result v3

    if-eqz v3, :cond_3

    add-int/lit8 v3, v0, -0x1

    if-ge v1, v3, :cond_5

    :cond_3
    const/4 v3, 0x1

    :goto_1
    invoke-direct {p0, v1, p1, v3}, Landroid/graphics/drawable/AnimationDrawable;->setFrame(IZZ)V

    .line 311
    return-void

    .line 302
    .restart local v2       #w:I
    :cond_4
    iget v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    iget v4, p0, Landroid/graphics/drawable/AnimationDrawable;->mIncX:I

    add-int/2addr v3, v4

    iput v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    .line 303
    iget v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    if-le v3, v2, :cond_2

    .line 305
    iput v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    goto :goto_0

    .line 310
    .end local v2           #w:I
    :cond_5
    const/4 v3, 0x0

    goto :goto_1
.end method

.method private setFrame(IZZ)V
    .locals 6
    .parameter "frame"
    .parameter "unschedule"
    .parameter "animate"

    .prologue
    .line 314
    iget-object v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    invoke-virtual {v2}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->getChildCount()I

    move-result v2

    if-lt p1, v2, :cond_1

    .line 367
    :cond_0
    :goto_0
    return-void

    .line 317
    :cond_1
    iput p1, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurFrame:I

    .line 319
    iget-boolean v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mIsScaledAnimate:Z

    if-nez v2, :cond_6

    .line 321
    invoke-virtual {p0, p1}, Landroid/graphics/drawable/AnimationDrawable;->selectDrawable(I)Z

    .line 328
    :goto_1
    if-eqz p2, :cond_2

    .line 329
    invoke-virtual {p0, p0}, Landroid/graphics/drawable/AnimationDrawable;->unscheduleSelf(Ljava/lang/Runnable;)V

    .line 330
    if-nez p3, :cond_2

    .line 339
    :cond_2
    iget-object v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    #getter for: Landroid/graphics/drawable/AnimationDrawable$AnimationState;->mDurations:[I
    invoke-static {v2}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->access$000(Landroid/graphics/drawable/AnimationDrawable$AnimationState;)[I

    move-result-object v2

    aget v0, v2, p1

    .line 341
    .local v0, time:I
    iget-boolean v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mIsScaledAnimate:Z

    if-eqz v2, :cond_5

    .line 343
    invoke-virtual {p0}, Landroid/graphics/drawable/AnimationDrawable;->getBounds()Landroid/graphics/Rect;

    move-result-object v2

    iget v2, v2, Landroid/graphics/Rect;->right:I

    invoke-virtual {p0}, Landroid/graphics/drawable/AnimationDrawable;->getBounds()Landroid/graphics/Rect;

    move-result-object v3

    iget v3, v3, Landroid/graphics/Rect;->left:I

    sub-int v1, v2, v3

    .line 344
    .local v1, w:I
    iget v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    iget v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    if-lt v2, v3, :cond_3

    iget v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    if-le v2, v1, :cond_4

    .line 346
    :cond_3
    iget v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    iput v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    .line 349
    :cond_4
    const/16 v0, 0x320

    .line 350
    iget v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    iget v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    if-ne v2, v3, :cond_7

    .line 352
    const/16 v0, 0x384

    .line 360
    .end local v1           #w:I
    :cond_5
    :goto_2
    if-eqz p3, :cond_0

    .line 364
    invoke-super {p0, p0}, Landroid/graphics/drawable/DrawableContainer;->unscheduleSelf(Ljava/lang/Runnable;)V

    .line 365
    invoke-static {}, Landroid/os/SystemClock;->uptimeMillis()J

    move-result-wide v2

    int-to-long v4, v0

    add-long/2addr v2, v4

    invoke-virtual {p0, p0, v2, v3}, Landroid/graphics/drawable/AnimationDrawable;->scheduleSelf(Ljava/lang/Runnable;J)V

    goto :goto_0

    .line 325
    .end local v0           #time:I
    :cond_6
    invoke-virtual {p0}, Landroid/graphics/drawable/AnimationDrawable;->invalidateSelf()V

    goto :goto_1

    .line 354
    .restart local v0       #time:I
    .restart local v1       #w:I
    :cond_7
    iget v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    if-ne v1, v2, :cond_5

    .line 356
    const/16 v0, 0x3e8

    goto :goto_2
.end method


# virtual methods
.method public addFrame(Landroid/graphics/drawable/Drawable;I)V
    .locals 2
    .parameter "frame"
    .parameter "duration"

    .prologue
    const/4 v1, 0x0

    .line 279
    iget-object v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    invoke-virtual {v0, p1, p2}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->addFrame(Landroid/graphics/drawable/Drawable;I)V

    .line 280
    iget v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurFrame:I

    if-gez v0, :cond_0

    .line 281
    const/4 v0, 0x1

    invoke-direct {p0, v1, v0, v1}, Landroid/graphics/drawable/AnimationDrawable;->setFrame(IZZ)V

    .line 283
    :cond_0
    return-void
.end method

.method public draw(Landroid/graphics/Canvas;)V
    .locals 4
    .parameter "canvas"

    .prologue
    .line 185
    iget-boolean v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mIsScaledAnimate:Z

    if-eqz v2, :cond_4

    .line 187
    iget-object v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurrDrawable:Landroid/graphics/drawable/Drawable;

    if-nez v2, :cond_0

    .line 189
    invoke-virtual {p0}, Landroid/graphics/drawable/AnimationDrawable;->getNumberOfFrames()I

    move-result v2

    if-lez v2, :cond_0

    .line 191
    const/4 v2, 0x0

    invoke-virtual {p0, v2}, Landroid/graphics/drawable/AnimationDrawable;->getFrame(I)Landroid/graphics/drawable/Drawable;

    move-result-object v2

    iput-object v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurrDrawable:Landroid/graphics/drawable/Drawable;

    .line 194
    :cond_0
    iget-object v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurrDrawable:Landroid/graphics/drawable/Drawable;

    if-eqz v2, :cond_3

    .line 196
    iget-object v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurrDrawable:Landroid/graphics/drawable/Drawable;

    if-eqz v2, :cond_3

    .line 197
    new-instance v0, Landroid/graphics/Rect;

    invoke-virtual {p0}, Landroid/graphics/drawable/AnimationDrawable;->getBounds()Landroid/graphics/Rect;

    move-result-object v2

    invoke-direct {v0, v2}, Landroid/graphics/Rect;-><init>(Landroid/graphics/Rect;)V

    .line 199
    .local v0, bound:Landroid/graphics/Rect;
    iget v2, v0, Landroid/graphics/Rect;->right:I

    iget v3, v0, Landroid/graphics/Rect;->left:I

    sub-int v1, v2, v3

    .line 200
    .local v1, w:I
    iget v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    iget v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    if-lt v2, v3, :cond_1

    iget v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    if-le v2, v1, :cond_2

    .line 202
    :cond_1
    iget v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    iput v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    .line 205
    :cond_2
    iget v2, v0, Landroid/graphics/Rect;->right:I

    iget v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    sub-int/2addr v2, v3

    iput v2, v0, Landroid/graphics/Rect;->left:I

    .line 206
    iget-object v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurrDrawable:Landroid/graphics/drawable/Drawable;

    invoke-virtual {v2, v0}, Landroid/graphics/drawable/Drawable;->setBounds(Landroid/graphics/Rect;)V

    .line 207
    iget-object v2, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurrDrawable:Landroid/graphics/drawable/Drawable;

    invoke-virtual {v2, p1}, Landroid/graphics/drawable/Drawable;->draw(Landroid/graphics/Canvas;)V

    .line 231
    .end local v0           #bound:Landroid/graphics/Rect;
    .end local v1           #w:I
    :cond_3
    :goto_0
    return-void

    .line 229
    :cond_4
    invoke-super {p0, p1}, Landroid/graphics/drawable/DrawableContainer;->draw(Landroid/graphics/Canvas;)V

    goto :goto_0
.end method

.method public enableScaleAnimation(Z)V
    .locals 0
    .parameter "enable"

    .prologue
    .line 110
    iput-boolean p1, p0, Landroid/graphics/drawable/AnimationDrawable;->mIsScaledAnimate:Z

    .line 111
    return-void
.end method

.method public getDuration(I)I
    .locals 1
    .parameter "i"

    .prologue
    .line 253
    iget-object v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    #getter for: Landroid/graphics/drawable/AnimationDrawable$AnimationState;->mDurations:[I
    invoke-static {v0}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->access$000(Landroid/graphics/drawable/AnimationDrawable$AnimationState;)[I

    move-result-object v0

    aget v0, v0, p1

    return v0
.end method

.method public getFrame(I)Landroid/graphics/drawable/Drawable;
    .locals 1
    .parameter "index"

    .prologue
    .line 245
    iget-object v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    invoke-virtual {v0}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->getChildren()[Landroid/graphics/drawable/Drawable;

    move-result-object v0

    aget-object v0, v0, p1

    return-object v0
.end method

.method public getNumberOfFrames()I
    .locals 1

    .prologue
    .line 238
    iget-object v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    invoke-virtual {v0}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->getChildCount()I

    move-result v0

    return v0
.end method

.method public inflate(Landroid/content/res/Resources;Lorg/xmlpull/v1/XmlPullParser;Landroid/util/AttributeSet;)V
    .locals 12
    .parameter "r"
    .parameter "parser"
    .parameter "attrs"
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lorg/xmlpull/v1/XmlPullParserException;,
            Ljava/io/IOException;
        }
    .end annotation

    .prologue
    const/4 v11, 0x2

    const/4 v10, 0x1

    const/4 v9, 0x0

    .line 373
    sget-object v7, Lcom/android/internal/R$styleable;->AnimationDrawable:[I

    invoke-virtual {p1, p3, v7}, Landroid/content/res/Resources;->obtainAttributes(Landroid/util/AttributeSet;[I)Landroid/content/res/TypedArray;

    move-result-object v0

    .line 376
    .local v0, a:Landroid/content/res/TypedArray;
    invoke-super {p0, p1, p2, v0, v9}, Landroid/graphics/drawable/DrawableContainer;->inflateWithAttributes(Landroid/content/res/Resources;Lorg/xmlpull/v1/XmlPullParser;Landroid/content/res/TypedArray;I)V

    .line 379
    iget-object v7, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    invoke-virtual {v0, v10, v9}, Landroid/content/res/TypedArray;->getBoolean(IZ)Z

    move-result v8

    invoke-virtual {v7, v8}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->setVariablePadding(Z)V

    .line 382
    iget-object v7, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    invoke-virtual {v0, v11, v9}, Landroid/content/res/TypedArray;->getBoolean(IZ)Z

    move-result v8

    #setter for: Landroid/graphics/drawable/AnimationDrawable$AnimationState;->mOneShot:Z
    invoke-static {v7, v8}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->access$102(Landroid/graphics/drawable/AnimationDrawable$AnimationState;Z)Z

    .line 385
    invoke-virtual {v0}, Landroid/content/res/TypedArray;->recycle()V

    .line 389
    invoke-interface {p2}, Lorg/xmlpull/v1/XmlPullParser;->getDepth()I

    move-result v7

    add-int/lit8 v5, v7, 0x1

    .line 391
    .local v5, innerDepth:I
    :cond_0
    :goto_0
    invoke-interface {p2}, Lorg/xmlpull/v1/XmlPullParser;->next()I

    move-result v6

    .local v6, type:I
    if-eq v6, v10, :cond_5

    invoke-interface {p2}, Lorg/xmlpull/v1/XmlPullParser;->getDepth()I

    move-result v1

    .local v1, depth:I
    if-ge v1, v5, :cond_1

    const/4 v7, 0x3

    if-eq v6, v7, :cond_5

    .line 393
    :cond_1
    if-ne v6, v11, :cond_0

    .line 397
    if-gt v1, v5, :cond_0

    invoke-interface {p2}, Lorg/xmlpull/v1/XmlPullParser;->getName()Ljava/lang/String;

    move-result-object v7

    const-string v8, "item"

    invoke-virtual {v7, v8}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v7

    if-eqz v7, :cond_0

    .line 401
    sget-object v7, Lcom/android/internal/R$styleable;->AnimationDrawableItem:[I

    invoke-virtual {p1, p3, v7}, Landroid/content/res/Resources;->obtainAttributes(Landroid/util/AttributeSet;[I)Landroid/content/res/TypedArray;

    move-result-object v0

    .line 402
    const/4 v7, -0x1

    invoke-virtual {v0, v9, v7}, Landroid/content/res/TypedArray;->getInt(II)I

    move-result v4

    .line 404
    .local v4, duration:I
    if-gez v4, :cond_2

    .line 405
    new-instance v7, Lorg/xmlpull/v1/XmlPullParserException;

    new-instance v8, Ljava/lang/StringBuilder;

    invoke-direct {v8}, Ljava/lang/StringBuilder;-><init>()V

    invoke-interface {p2}, Lorg/xmlpull/v1/XmlPullParser;->getPositionDescription()Ljava/lang/String;

    move-result-object v9

    invoke-virtual {v8, v9}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v8

    const-string v9, ": <item> tag requires a \'duration\' attribute"

    invoke-virtual {v8, v9}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v8

    invoke-virtual {v8}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v8

    invoke-direct {v7, v8}, Lorg/xmlpull/v1/XmlPullParserException;-><init>(Ljava/lang/String;)V

    throw v7

    .line 409
    :cond_2
    invoke-virtual {v0, v10, v9}, Landroid/content/res/TypedArray;->getResourceId(II)I

    move-result v3

    .line 412
    .local v3, drawableRes:I
    invoke-virtual {v0}, Landroid/content/res/TypedArray;->recycle()V

    .line 415
    if-eqz v3, :cond_3

    .line 416
    invoke-virtual {p1, v3}, Landroid/content/res/Resources;->getDrawable(I)Landroid/graphics/drawable/Drawable;

    move-result-object v2

    .line 429
    .local v2, dr:Landroid/graphics/drawable/Drawable;
    :goto_1
    iget-object v7, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    invoke-virtual {v7, v2, v4}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->addFrame(Landroid/graphics/drawable/Drawable;I)V

    .line 430
    if-eqz v2, :cond_0

    .line 431
    invoke-virtual {v2, p0}, Landroid/graphics/drawable/Drawable;->setCallback(Landroid/graphics/drawable/Drawable$Callback;)V

    goto :goto_0

    .line 418
    .end local v2           #dr:Landroid/graphics/drawable/Drawable;
    :cond_3
    invoke-interface {p2}, Lorg/xmlpull/v1/XmlPullParser;->next()I

    move-result v6

    const/4 v7, 0x4

    if-eq v6, v7, :cond_3

    .line 421
    if-eq v6, v11, :cond_4

    .line 422
    new-instance v7, Lorg/xmlpull/v1/XmlPullParserException;

    new-instance v8, Ljava/lang/StringBuilder;

    invoke-direct {v8}, Ljava/lang/StringBuilder;-><init>()V

    invoke-interface {p2}, Lorg/xmlpull/v1/XmlPullParser;->getPositionDescription()Ljava/lang/String;

    move-result-object v9

    invoke-virtual {v8, v9}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v8

    const-string v9, ": <item> tag requires a \'drawable\' attribute or child tag"

    invoke-virtual {v8, v9}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v8

    const-string v9, " defining a drawable"

    invoke-virtual {v8, v9}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v8

    invoke-virtual {v8}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v8

    invoke-direct {v7, v8}, Lorg/xmlpull/v1/XmlPullParserException;-><init>(Ljava/lang/String;)V

    throw v7

    .line 426
    :cond_4
    invoke-static {p1, p2, p3}, Landroid/graphics/drawable/Drawable;->createFromXmlInner(Landroid/content/res/Resources;Lorg/xmlpull/v1/XmlPullParser;Landroid/util/AttributeSet;)Landroid/graphics/drawable/Drawable;

    move-result-object v2

    .restart local v2       #dr:Landroid/graphics/drawable/Drawable;
    goto :goto_1

    .line 435
    .end local v1           #depth:I
    .end local v2           #dr:Landroid/graphics/drawable/Drawable;
    .end local v3           #drawableRes:I
    .end local v4           #duration:I
    :cond_5
    invoke-direct {p0, v9, v10, v9}, Landroid/graphics/drawable/AnimationDrawable;->setFrame(IZZ)V

    .line 437
    invoke-virtual {p0, v9}, Landroid/graphics/drawable/AnimationDrawable;->getFrame(I)Landroid/graphics/drawable/Drawable;

    move-result-object v7

    iput-object v7, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurrDrawable:Landroid/graphics/drawable/Drawable;

    .line 438
    return-void
.end method

.method public isOneShot()Z
    .locals 1

    .prologue
    .line 260
    iget-object v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    #getter for: Landroid/graphics/drawable/AnimationDrawable$AnimationState;->mOneShot:Z
    invoke-static {v0}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->access$100(Landroid/graphics/drawable/AnimationDrawable$AnimationState;)Z

    move-result v0

    return v0
.end method

.method public isRunning()Z
    .locals 2

    .prologue
    .line 163
    iget v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurFrame:I

    const/4 v1, -0x1

    if-le v0, v1, :cond_0

    const/4 v0, 0x1

    :goto_0
    return v0

    :cond_0
    const/4 v0, 0x0

    goto :goto_0
.end method

.method public mutate()Landroid/graphics/drawable/Drawable;
    .locals 2

    .prologue
    .line 442
    iget-boolean v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mMutated:Z

    if-nez v0, :cond_0

    invoke-super {p0}, Landroid/graphics/drawable/DrawableContainer;->mutate()Landroid/graphics/drawable/Drawable;

    move-result-object v0

    if-ne v0, p0, :cond_0

    .line 443
    iget-object v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    iget-object v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    #getter for: Landroid/graphics/drawable/AnimationDrawable$AnimationState;->mDurations:[I
    invoke-static {v0}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->access$000(Landroid/graphics/drawable/AnimationDrawable$AnimationState;)[I

    move-result-object v0

    invoke-virtual {v0}, [I->clone()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, [I

    #setter for: Landroid/graphics/drawable/AnimationDrawable$AnimationState;->mDurations:[I
    invoke-static {v1, v0}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->access$002(Landroid/graphics/drawable/AnimationDrawable$AnimationState;[I)[I

    .line 444
    const/4 v0, 0x1

    iput-boolean v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mMutated:Z

    .line 446
    :cond_0
    return-object p0
.end method

.method protected onBoundsChange(Landroid/graphics/Rect;)V
    .locals 3
    .parameter "bounds"

    .prologue
    .line 479
    invoke-virtual {p0}, Landroid/graphics/drawable/AnimationDrawable;->getBounds()Landroid/graphics/Rect;

    move-result-object v1

    iget v1, v1, Landroid/graphics/Rect;->right:I

    invoke-virtual {p0}, Landroid/graphics/drawable/AnimationDrawable;->getBounds()Landroid/graphics/Rect;

    move-result-object v2

    iget v2, v2, Landroid/graphics/Rect;->left:I

    sub-int v0, v1, v2

    .line 480
    .local v0, w:I
    iget v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseLevel:I

    rsub-int/lit8 v1, v1, 0x64

    mul-int/2addr v1, v0

    div-int/lit8 v1, v1, 0x64

    sub-int v1, v0, v1

    iput v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    .line 481
    iget v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    if-gez v1, :cond_0

    .line 483
    const/4 v1, 0x0

    iput v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    .line 486
    :cond_0
    invoke-super {p0, p1}, Landroid/graphics/drawable/DrawableContainer;->onBoundsChange(Landroid/graphics/Rect;)V

    .line 487
    return-void
.end method

.method protected onLevelChange(I)Z
    .locals 4
    .parameter "level"

    .prologue
    const/4 v3, 0x1

    .line 452
    iput p1, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseLevel:I

    .line 453
    invoke-virtual {p0}, Landroid/graphics/drawable/AnimationDrawable;->getBounds()Landroid/graphics/Rect;

    move-result-object v1

    iget v1, v1, Landroid/graphics/Rect;->right:I

    invoke-virtual {p0}, Landroid/graphics/drawable/AnimationDrawable;->getBounds()Landroid/graphics/Rect;

    move-result-object v2

    iget v2, v2, Landroid/graphics/Rect;->left:I

    sub-int v0, v1, v2

    .line 456
    .local v0, w:I
    mul-int v1, v0, p1

    add-int/lit8 v1, v1, 0x32

    div-int/lit8 v1, v1, 0x64

    iput v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    .line 457
    if-lez p1, :cond_3

    iget v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    if-ge v1, v3, :cond_3

    .line 459
    iput v3, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    .line 465
    :cond_0
    :goto_0
    iget v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    if-le v1, v0, :cond_1

    .line 466
    iput v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    .line 469
    :cond_1
    iget v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    if-gez v1, :cond_2

    .line 470
    const/4 v1, 0x0

    iput v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    .line 473
    :cond_2
    invoke-super {p0, p1}, Landroid/graphics/drawable/DrawableContainer;->onLevelChange(I)Z

    move-result v1

    return v1

    .line 460
    :cond_3
    const/16 v1, 0x64

    if-ge p1, v1, :cond_0

    iget v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    if-lt v1, v0, :cond_0

    .line 462
    add-int/lit8 v1, v0, -0x1

    iput v1, p0, Landroid/graphics/drawable/AnimationDrawable;->mBaseWidth:I

    goto :goto_0
.end method

.method public run()V
    .locals 1

    .prologue
    .line 173
    const/4 v0, 0x0

    invoke-direct {p0, v0}, Landroid/graphics/drawable/AnimationDrawable;->nextFrame(Z)V

    .line 174
    return-void
.end method

.method setIncX(I)V
    .locals 0
    .parameter "x"

    .prologue
    .line 497
    iput p1, p0, Landroid/graphics/drawable/AnimationDrawable;->mIncX:I

    .line 498
    return-void
.end method

.method public setDuration(II)V
    .locals 1
    .parameter "i"
    .parameter "duration"

    .prologue
    iget-object v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    #getter for: Landroid/graphics/drawable/AnimationDrawable$AnimationState;->mDurations:[I
    invoke-static {v0}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->access$000(Landroid/graphics/drawable/AnimationDrawable$AnimationState;)[I

    move-result-object v0

    aput p2, v0, p1

    return-void
.end method

.method public setFrame(ILandroid/graphics/drawable/Drawable;)V
    .locals 1
    .parameter "index"
    .parameter "drawable"

    .prologue
    iget-object v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    invoke-virtual {v0}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->getChildren()[Landroid/graphics/drawable/Drawable;

    move-result-object v0

    aput-object p2, v0, p1

    return-void
.end method

.method public setOneShot(Z)V
    .locals 1
    .parameter "oneShot"

    .prologue
    .line 269
    iget-object v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mAnimationState:Landroid/graphics/drawable/AnimationDrawable$AnimationState;

    #setter for: Landroid/graphics/drawable/AnimationDrawable$AnimationState;->mOneShot:Z
    invoke-static {v0, p1}, Landroid/graphics/drawable/AnimationDrawable$AnimationState;->access$102(Landroid/graphics/drawable/AnimationDrawable$AnimationState;Z)Z

    .line 270
    return-void
.end method

.method public setVisible(ZZ)Z
    .locals 3
    .parameter "visible"
    .parameter "restart"

    .prologue
    const/4 v2, 0x1

    .line 115
    invoke-super {p0, p1, p2}, Landroid/graphics/drawable/DrawableContainer;->setVisible(ZZ)Z

    move-result v0

    .line 116
    .local v0, changed:Z
    if-eqz p1, :cond_2

    .line 117
    if-nez v0, :cond_0

    if-eqz p2, :cond_1

    .line 118
    :cond_0
    const/4 v1, 0x0

    invoke-direct {p0, v1, v2, v2}, Landroid/graphics/drawable/AnimationDrawable;->setFrame(IZZ)V

    .line 123
    :cond_1
    :goto_0
    return v0

    .line 121
    :cond_2
    invoke-virtual {p0, p0}, Landroid/graphics/drawable/AnimationDrawable;->unscheduleSelf(Ljava/lang/Runnable;)V

    goto :goto_0
.end method

.method public start()V
    .locals 1

    .prologue
    .line 139
    invoke-virtual {p0}, Landroid/graphics/drawable/AnimationDrawable;->isRunning()Z

    move-result v0

    if-nez v0, :cond_0

    .line 140
    invoke-virtual {p0}, Landroid/graphics/drawable/AnimationDrawable;->run()V

    .line 142
    :cond_0
    return-void
.end method

.method public stop()V
    .locals 1

    .prologue
    .line 152
    invoke-virtual {p0}, Landroid/graphics/drawable/AnimationDrawable;->isRunning()Z

    move-result v0

    if-eqz v0, :cond_0

    .line 153
    invoke-virtual {p0, p0}, Landroid/graphics/drawable/AnimationDrawable;->unscheduleSelf(Ljava/lang/Runnable;)V

    .line 155
    :cond_0
    return-void
.end method

.method public unscheduleSelf(Ljava/lang/Runnable;)V
    .locals 1
    .parameter "what"

    .prologue
    const/4 v0, -0x1

    .line 178
    iput v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurFrame:I

    .line 179
    iput v0, p0, Landroid/graphics/drawable/AnimationDrawable;->mCurWidth:I

    .line 180
    invoke-super {p0, p1}, Landroid/graphics/drawable/DrawableContainer;->unscheduleSelf(Ljava/lang/Runnable;)V

    .line 181
    return-void
.end method
